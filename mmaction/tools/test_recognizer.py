...
def parse_args():
    ...
    parser.add_argument('--name', help='output result file', default='default.pkl')
...
def main():
    ...
    model = MMDataParallel(model, device_ids=range(args.test_gpus,args.test_gpus+1))
    ...
    if args.out and rank == 0:
        ...
        for i in range(len(results)):
            ...
            index = prediction[(prediction[0]==image_name[i].replace("_{}".format(args.color_or_depth),""))].index.item()
            # print("{}:{}".format(index,pred_class))
            prediction.iloc[index,1] = pred_class 
            pbar.update(1)
            # if i==5:
            #     break
        prediction.to_csv(os.path.join(args.out,'{}_predictions.csv'.format(args.name)),header=False,index=False)
