...
class GroupImageTransform(object):
    ...
    def __init__(self,
        ...
        self.afterdetect_resize = afterdetect_resize
        # croping parameters
        if crop_size is not None:
            if oversample == 'three_crop':
                ...
                # self.op_crop = RandomResizedCrop(crop_size,scale=(0.8,1.0))
                self.op_crop = RandomResizedCrop(crop_size,scale=RandomResizeCrop_scale)
            elif rescale_crop:
    ...
    def __call__(self, img_group, scale, crop_history=None, flip=False,
                 keep_ratio=True, div_255=False, is_flow=False, image_name=None):

        if self.resize_crop or self.rescale_crop:
            if self.afterdetect_resize:
                img_group = [cv2.resize(img,self.afterdetect_resize) for img in img_group]
