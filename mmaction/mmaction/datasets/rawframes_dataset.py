...
class RawFramesDataset(Dataset):
    def __init__(self,
                 ...
                 mask=False,
                 bbox_label=None,
                 afterdetect_resize=None,
                 RandomResizeCrop_scale=(0.08, 1.0)):
        # prefix of images path
        self.img_prefix = img_prefix
        self.mask = mask
        self.bbox_label = pd.read_csv(bbox_label,header=None) if bbox_label is not None else None
        # load annotations
        ...
        self.img_group_transform = GroupImageTransform(
            ...
            RandomResizeCrop_scale=RandomResizeCrop_scale,
            afterdetect_resize=afterdetect_resize,
            **self.img_norm_cfg)
    ...
    def _load_image(self, directory, image_tmpl, modality, idx):
        if modality in ['RGB', 'RGBDiff']:
            img = mmcv.imread(osp.join(directory, image_tmpl.format(idx)))
            if not self.mask:
                if self.bbox_label is not None:
                    # if not self.test_mode:
                    #     class_sample = os.path.join(directory.split("/")[-2],directory.split("/")[-1])
                    #     index = self.bbox_label[(self.bbox_label[0]==class_sample)].index.item()
                    # else:
                    #     sample = directory.split("/")[-1]
                    #     index = self.bbox_label[(self.bbox_label[0]==sample)].index.item()
                    sample = directory.split("/")[-1]
                    index = self.bbox_label[(self.bbox_label[0]==sample)].index.item()
                    bbox = self.bbox_label.iloc[index,1:5]
                    bbox = [int(bbox.iloc[i]) for i in range(len(bbox))]
                    x1,y1,x2,y2 = bbox
                    img = img[y1:y2,x1:x2]
                    # import matplotlib.pyplot as plt
                    # plt.imshow(img)
                    # plt.savefig("show_fig/{}".format(sample))
                    # import pdb;pdb.set_trace()
                    return [img]
                else:
                    return [img]
            else:
                img_mask = mmcv.imread(osp.join(directory.replace("RGB","depth").replace("color","depth"),image_tmpl.format(idx)))
