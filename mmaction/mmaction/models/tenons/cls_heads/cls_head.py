...
class ClsHead(nn.Module):
    ...
    def loss(self,
        ...
        labels = np.array(labels.detach().cpu().numpy())[:, np.newaxis]

        max_1_preds = np.argsort(cls_score.detach().cpu().numpy(), axis=1)[:, -1:][:, ::-1]
        match_array = np.logical_or.reduce(max_1_preds == labels, axis=1)
        top_1_acc = match_array.sum() / match_array.shape[0]


        losses['top1_acc'] = torch.tensor(
            top_1_acc, device=cls_score.device)
