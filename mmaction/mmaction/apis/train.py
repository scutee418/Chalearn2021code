...
def batch_processor(model, data, train_mode):
    '''
    for tsn:  (input format: N C H W )
    data['img_group_0'].data[0].shape: torch.Size([16, 12, 3, 224, 224])
    batch_size * num_segment * channel * H * W

    for slowonly: (input format: N C T H W )
    data['img_group_0'].data[0].shape: torch.Size([12, 1, 3, 8, 224, 224])
    batch_size * (num_oversample*num_segment) * channel * new_length * H * W
    '''
    losses = model(**data)
