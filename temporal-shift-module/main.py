...
def main():
    ...
    if args.tune_from:
        ...
        sd = torch.load(args.tune_from,map_location="cuda:1")
        sd = sd['state_dict']
    ...
    if args.loss_type == 'nll':
        criterion = torch.nn.CrossEntropyLoss().cuda(1)
    else:
...
def save_checkpoint(state, is_best, epoch):
    filename = '%s/%s/ckpt.%spth.tar' % (args.root_model, args.store_name, str(epoch+1))
