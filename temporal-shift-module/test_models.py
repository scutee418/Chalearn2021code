...
for this_weights, this_test_segments, test_file in zip(weights_list, test_segments_list, test_file_list):
    ...
    num_class, args.train_list, val_list, test_list, root_path, prefix = dataset_config.return_dataset(args.dataset,
                                                                                            modality)
    ...
    checkpoint = torch.load(this_weights,map_location="cuda:1")
    ...
    test_dataset = TSNDataSet(root_path, test_file if test_file is not None else val_list, num_segments=this_test_segments,
    ...
    net = torch.nn.DataParallel(net,device_ids=args.gpus).cuda(args.gpus[0])
    net.eval()
...
for i, data_label_pairs in enumerate(zip(*data_iter_list)):
    with torch.no_grad():
        ...
        for n_seg, (_, (data, label)), net, modality in zip(test_segments_list, data_label_pairs, net_list, modality_list):
            ...
            results_numpyformat = np.row_stack((results_numpyformat,rst[1]))
    # break
    pbar.update(1)
...
for i in  range(len(video_list)):
    image_name.append(video_list[i].path)
